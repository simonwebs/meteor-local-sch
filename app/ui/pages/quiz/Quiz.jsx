    ...
    <div className="dark:bg-gradient-to-l from-sky-800 to-cyan-700 rounded-2xl max-w-md w-full space-y-8">
      ...
      {quizComplete ? (
       ...
       <Fragment>
        ...
        {answerStatus != null && (
          <div>
            <div className="mt-2 max-w-3xl text-xl text-gray-300">{!!answerStatus ? "Correct! :)" : "Your answer was incorrect :("}</div>
            <button className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" onClick={onNextClick}>
      ...
      {questionIndex != null && <button className="inline-flex items-center mt-5  px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none" onClick={onRestartClick}>Restart quiz</button>}
    </div>
    </div>
  )
}
